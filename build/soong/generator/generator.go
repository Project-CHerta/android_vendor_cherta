...
func init() {
	android.RegisterModuleType("cherta_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = chertaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = chertaExpandVariables(ctx, dep_file)
	...
	cmd := chertaExpandVariables(ctx, String(g.properties.Cmd))
